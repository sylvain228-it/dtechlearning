...
import { logout } from '@/routes/partner';
...
export function PartnerSidebarHeader() {
    const page = usePage<PartnerSharedData>();
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Avatar className="size-8 cursor-pointer overflow-hidden rounded-full">
                            {auth.partner.logo_url != null && (
                                <AvatarImage
                                    src={auth.partner.logo_url}
                                    alt={auth.partner.name}
                                />
                            )}
                            <AvatarFallback className="rounded-lg bg-neutral-200 text-black dark:bg-neutral-700 dark:text-white">
                                {getInitials(auth.partner.name)}
                            </AvatarFallback>
                        </Avatar>
                    </DropdownMenuTrigger>
